...
bool GameWidget::findFont(QString name)
{
    fontName = "";
    QFont font(name);
    if (font.family() != name)
...
void GameWidget::onMouseLongPress(QPoint point, QSize size)
{
    emit popupMenu();
}

void GameWidget::refreshScreen()
{
    if (flashScreen)
    {
        flashScreen = false;
